...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    try {
      ...
      setWebglError("WebGL is not available in this browser. Enable hardware acceleration in chrome://settings/system and relaunch.");
      return;
    }
  ...
  if (webglError) {
    return (
      <div className={cn("relative h-full w-full flex items-center justify-center bg-muted text-muted-foreground text-sm p-6 text-center", className)}>
        <p>{webglError}</p>
      </div>
    );
  }
