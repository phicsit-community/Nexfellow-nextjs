...
const BlogWriter: React.FC<BlogWriterProps> = ({ blogToEdit, onCancel, onSave }) => {
    ...
    return (
        <div className="bg-slate-800 rounded-xl p-6">
            <div className="mb-6">
                <h2 className="text-xl font-semibold text-white">{blogToEdit ? 'Edit Blog' : 'Write a Blog'}</h2>
                <p className="text-slate-400 text-sm">Create and publish new content</p>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <input
                        ...
                        className="px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400"
                        value={category}
                ...
                <div>
                    <label className="block text-slate-300 text-sm mb-2">Cover Image</label>
                    <input
                        type="file"
                        accept="image/*"
                ...
                <div className="bg-slate-700/50 rounded-lg p-4">
                    <h3 className="text-white font-medium mb-3">Author Information</h3>
                    <div className="grid grid-cols-2 gap-4">
                        <input
                            type="text"
                            placeholder="Author Name"
                            className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400"
                            value={authorName}
                    ...
                    <textarea
                        ...
                        className="w-full mt-3 px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 resize-none"
                        rows={2}
                        value={authorBio}
                        onChange={(e) => setAuthorBio(e.target.value)}
                    />
                    <div className="mt-3">
                        <label className="block text-slate-400 text-sm mb-1">Author Image</label>
                        <input
                            type="file"
                            accept="image/*"
                            className="text-slate-400"
                ...
                <div className="flex gap-2 mb-2">
                    <button
                        className={`px-4 py-2 rounded-lg ${!showPreview ? 'bg-teal-500 text-white' : 'bg-slate-700 text-slate-300'}`}
                ...
                {!showPreview ? (
                    <textarea
                        ...
                        className="w-full min-h-[300px] px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 resize-y font-mono"
                        value={content}
                ...
                <div className="flex gap-3 mt-6">
                    <button
                        ...
                        disabled={saving}
                        className="px-6 py-2 bg-slate-600 hover:bg-slate-500 text-white rounded-lg transition-colors disabled:opacity-50"
                    >
                        Save as Draft
                    </button>
                    <button
    ...
    return (
        <div className="min-h-screen p-6">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-2xl font-semibold text-white">All Blogs</h1>
                    <p className="text-slate-400">Manage and moderate blog posts</p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                ...
                <div className="bg-slate-700 rounded-lg p-4">
                    ...
                    <p className="text-2xl font-bold text-green-400">{blogs.filter((b) => b.status === 'published').length}</p>
                </div>
                <div className="bg-slate-700 rounded-lg p-4">
                    <p className="text-slate-400 text-sm">Drafts</p>
                    <p className="text-2xl font-bold text-yellow-400">{blogs.filter((b) => b.status === 'draft').length}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    ...
                    filteredBlogs.map((blog) => (
                        <div key={blog._id} className="bg-slate-700 rounded-lg overflow-hidden">
                            {blog.coverUrl && (
                            ...
                            <div className="p-4">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-xs bg-slate-600 px-2 py-1 rounded text-slate-300">{blog.category || 'Uncategorized'}</span>
                                    <span className={`text-xs px-2 py-1 rounded ${blog.status === 'published' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                                        {blog.status}
                                    </span>
                                </div>
                                ...
                                {blog.tags && blog.tags.length > 0 && (
                                    <div className="flex flex-wrap gap-1 mb-2">
                                        {blog.tags.slice(0, 3).map((tag, i) => (
                                            <span key={i} className="text-xs text-teal-400">#{tag}</span>
                                        ))}
                                    </div>
                                )}
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="flex items-center gap-1 text-xs bg-slate-600 hover:bg-slate-500 text-white px-3 py-1.5 rounded transition-colors"
                                    >
                                        <FiEdit2 className="w-3 h-3" /> Edit
                                    </button>
                                    <button
                                        onClick={() => handleAction(blog._id, 'delete')}
                                        className="flex items-center gap-1 text-xs bg-red-500/20 hover:bg-red-500/30 text-red-400 px-3 py-1.5 rounded transition-colors"
                                    >
                                        <FiTrash2 className="w-3 h-3" /> Delete
                                    </button>
                                    <button
                                        onClick={() => handleAction(blog._id, blog.status === 'published' ? 'unpublish' : 'publish')}
                                        className="text-xs bg-teal-500/20 hover:bg-teal-500/30 text-teal-400 px-3 py-1.5 rounded transition-colors"
                                    >
