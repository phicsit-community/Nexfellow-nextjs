const translateCronToReadable = (cronExpression: string): string => {
  ...
  const formattedDayOfMonth =
    dayOfMonth === "*"
      ? ""
      : `${dayOfMonth}${dayOfMonth !== "1" ? "th " : "st "}`;
  const formattedMonth =
    month === "*" ? "" : " " + `${getMonthName(parseInt(month))}`;
  const formattedDayOfWeek =
    dayOfWeek === "*" ? "" : " " + `${getDayName(parseInt(dayOfWeek))}`;
  ...
  return schedule.replace(/\s{2,}/g, " ").trim();
};
